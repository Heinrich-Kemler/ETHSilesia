  ...
  boldParts.forEach((chunk, bi) => {
    ...
    italicParts.forEach((piece, ii) => {
      if (
        ...
        nodes.push(<em key={`i-${bi}-${ii}`}>{piece.slice(1, -1)}</em>);
      } else if (piece) {
...
export default function ChatPanel({
  ...
  useEffect(() => {
    ...
    async function loadHistory() {
      // Only show loading spinner if we don't have enough messages yet
      if (messages.length <= 1) setLoadingHistory(true);
  ...
  const suggestions: Array<
    "chatSuggestion1" | "chatSuggestion2" | "chatSuggestion3"
  > = ["chatSuggestion1", "chatSuggestion2", "chatSuggestion3"];
          ...
          >
            ...
            {loadingHistory && (
              <div className="flex justify-center py-4">
                <div className="w-5 h-5 border-2 border-gold-themed/30 border-t-gold-themed rounded-full animate-spin" />
              </div>
            )}
